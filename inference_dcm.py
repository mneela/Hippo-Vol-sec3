...
def get_predicted_volumes(pred):
    ...
    # <YOUR CODE HERE>
    # there are two class labels here
    # class label 1 == anterior
    # class label 2 == posterior
...
def get_series_for_inference(path):
    ...
    # commenting the below code as it is not readng all the dicom objects
    # we observe that right series (directory) contains HippoCropVolume
    #print(path)
    series_path = [dir for dir, subdirs, files in os.walk(path) if 'HCropVolume' in dir]
    #print(series_path)
    #print(type(series_path[0]))
    #input("pause 4")
    s_path = Path(series_path[0])
    #s_path = '/data/TestVolumes/Study1/13_HCropVolume/'
    # dicoms = [pydicom.dcmread(os.path.join(path, f)) for f in os.listdir(path)]
    dicoms = [pydicom.dcmread(os.path.join(s_path, f)) for f in os.listdir(s_path)]
    ...
    # the series description of the dicom image have "HippoCrop"
    # we need only these series for inference
    # Check if there are more than one series (using set comprehension).
    #series_for_inference = [dicoms[i] for i in range(len(dicoms)) if "HippoCrop" in dicoms[i].SeriesDescription ]
    ...
    print(f"returning series_for_inference series with len : {len(series_for_inference)}")
...
if __name__ == "__main__":
    ...
    volume, header = load_dicom_volume_as_numpy_from_list(get_series_for_inference(sys.argv[1]))
    ...
    report_save_path = r"/home/workspace/out/report_study2.dcm"
    ...
    # followed the medium link : https://medium.com/@GaganaB/clinical-workflow-integration-with-orthanc-and-ohif-viewer-38930a3e94de
    cmd = 'storescu 127.0.0.1 4242 -v -aec HIPPOAI +r +sd /home/workspace/out/report_study2.dcm'
    os_command(cmd)
